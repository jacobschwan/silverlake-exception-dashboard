...
get_trip_data <- function() {
   ...
   trips <- dbGetQuery(core, "SELECT DISTINCT TRIPNUM FROM EXCEPT") %>% pull(TRIPNUM)
   
   ptrips <- dbGetQuery(core, "SELECT DISTINCT PTRIPNUM FROM EXCEPT") %>% pull(PTRIPNUM)
   
   dbDisconnect(core)
   
   if(ptrips == "" | length(ptrips == 0)) {
     default <- last(trips)
   } else {
     default <- trips[!(trips %in% ptrips)]
   }
...
all_query_data <- reactive({
   ...
   all_data <- dbGetQuery(core, "SELECT TRIPNUM, APPPRFX, APPNAME, APPNUM, FDESC1, FDESC2, VERTEAM, PROBLEM, RESOLVED, APPROVED,
                          ...
                          FROM EXCEPT") %>%
    mutate(DATA_REFRESH = as.POSIXct(DATA_REFRESH, tz = "America/Chicago"))
   dbDisconnect(core)
   return(all_data)
   })
